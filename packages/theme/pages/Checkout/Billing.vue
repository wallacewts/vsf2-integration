    ...
    <form @submit.prevent="handleSubmit(handleAddressSubmit(reset))">
      ...
      <div class="form" v-if="canAddNewAddress">
        ...
        <ValidationProvider name="zipCode" rules="required|length:9" v-slot="{ errors }" slim>
          <SfInput
            ...
            v-mask="'99999-999'"
            @input="(postalCode) => handleZipCode(postalCode)"
            :label="$t('Zip-code')"
            name="zipCode"
            class="form__element form__element--half"
            required
        ...
        <ValidationProvider name="country" rules="required|min:2" v-slot="{ errors }" slim>
          <SfSelect
            ...
            name="country"
            class="
              form__element form__element--half form__element--half-even form__select
      ...
      <div class="form">
        <div class="form__action">
          <SfButton class="form__action-button" type="submit" :disabled="!canMoveForward">
...
export default {
  ...
  setup(_, context) {
    const { searchLocale, cidade, logradouro } = useSearchLocale();
    ...
    const canMoveForward = computed(
      () => !loading.value && billingDetails.value && Object.keys(billingDetails.value).length
    );
    ...
    const statesInSelectedCountry = computed(() => {
      ...
      const selectedCountry = config.countries.find(
        (country) => country.name === billingDetails.value.country
      );
    ...
    watch([logradouro, cidade], ([logradouro, cidade]) => {
      changeBillingDetails('streetName', logradouro);
      changeBillingDetails('city', cidade);
    });
    ...
    const handleZipCode = (zipCode) => {
      changeBillingDetails('postalCode', zipCode);
