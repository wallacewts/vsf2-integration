  ...
  <div>
      ...
      <SfAccordionItem :header="$t('Shipping address')">
        <div class="accordion__item">
          <div v-e2e="'payment-shipping-address'" class="accordion__content">
            <p class="content">
              <span class="content__label" v-if="chosenShippingMethod">{{
    ...
    <div v-e2e="'payment-summary'" class="summary">
      <div class="summary__group">
        ...
        />
        <VsfPaymentProviderMock :transactionProduct="transactionProduct" />
      </div>
    </div>
  </div>
</template>
...
import { useUiNotification } from '@vue-storefront/exedio-omnicommerce';
...
export default {
  ...
  setup(_, context) {
    ...
    const billingSameAsShipping = computed(() =>
      Object.keys(shippingDetails.value).every(
        (shippingDetailsKey) =>
          shippingDetails.value[shippingDetailsKey] === billingDetails.value[shippingDetailsKey]
      )
    );
    ...
    products.value.forEach((product) => {
      const { fractionDigits, centAmount } = product.price.value;
      const productToTransaction = {
        description: product.name,
        quantity: product.quantity,
        price_unit: centAmount / Math.pow(10, fractionDigits),
        code: product.id,
        sku_code: product.productId,
        extra: ''
      };
      transactionProduct.push(productToTransaction);
    });
    return {
      loading,
      products,
      shippingDetails,
      billingDetails,
      chosenShippingMethod: computed(
        () => state.value && state.value.response && state.value.response.shippingMethod
      ),
      chosenPaymentMethod: {},
