        ...
        <div class="products" v-if="!loading">
          ...
          <transition-group v-else appear name="products__slide" tag="div" class="products__list">
            <SfProductCardHorizontal
              ...
              "
              @click:add-to-cart="
                addToCart({ product, quantity: Number(productsQuantity[product._id]) })
              "
            >
          ...
          <div v-show="pagination.totalPages > 1" class="products__show-on-page">
            ...
            <LazyHydrate on-interaction>
              <SfSelect
                :value="
                  pagination && pagination.itemsPerPage ? pagination.itemsPerPage.toString() : ''
                "
...
import { useUiHelpers, useUiState } from '@vue-storefront/exedio-omnicommerce';
  ...
  setup(props, context) {
    ...
    const activeCategory = computed(() => {
      ...
      const category = items.find(
        ({ isCurrent, items }) => isCurrent || items.find(({ isCurrent }) => isCurrent)
      );
